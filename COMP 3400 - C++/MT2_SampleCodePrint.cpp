...
//Part 2
    ...
    std::stable_sort(numbers.begin(),numbers.end(),[](point a, point b){return a < b;});
    std::for_each(coordinates.begin(), coordinates.end(), [](auto e){std::cout<<e;});
    
    //Last 3 Points
    std::cout<<"hello world"; //std::cout
    std::sort(numbers.begin(), numbers.end(), [](int a, int b){return a < b;}); //Passing lambda function to an algorithm
    if(numbers.empty()) //Throw exception for empty vector
        throw "Empty vector";